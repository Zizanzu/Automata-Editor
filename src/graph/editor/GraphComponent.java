...
public class GraphComponent extends JComponent implements MouseInputListener, KeyListener {
	...
	private int dx = 0;
	private int dy = 0;
	private static final Color[] colorList = new Color[] {	Color.GRAY };
		...
		try {
			...
			observable.addObserver(new Observer() {
				public void update(Observable observable, Object edge) {
					Edge e = (Edge) edge;
					System.out.println("State " + e.source.label + " go '" + e.getLabel() + "' to State " + e.target.label);
				}
			});
		...
		try {
			...
			observable.addObserver(new Observer() {
				public void update(Observable observable, Object edge) {
					Edge e = (Edge) edge;
					//System.out.println("State " + e.source.label + " go '" + e.getLabel() + "' to State " + e.target.label);
                                        edgeList.add(e.source.label);
                                        edgeList.add(e.getLabel());
                                        edgeList.add(e.target.label);
                                        // Checking if an element is included to the list
                                        boolean element = edgeList.contains(e.source.label);
                                        if (element == false) {
                                            vertexList.add(e.source.label);
                                        }                                         
                                }
			});
		} catch (NotDeterministInitialStateException e) {
